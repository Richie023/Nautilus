...
// ─── Labels por tipo de conector ──────────────────────────────────────────────
// Agrega aquí cualquier conector que necesite labels personalizados en apikey
const CREDENTIAL_LABELS: Record<string, {
  username?: string
  usernamePlaceholder?: string
  apiKey?: string
  apiKeyPlaceholder?: string
  apiKeyHeader?: string
  apiKeyHeaderPlaceholder?: string
  apiKeyHeaderHint?: string
}> = {
  threatdown: {
    username: 'Client ID',
    usernamePlaceholder: 'tu-client-id',
    apiKey: 'Client Secret',
    apiKeyPlaceholder: 'tu-client-secret',
    apiKeyHeader: 'Account ID',
    apiKeyHeaderPlaceholder: 'tu-account-id',
    apiKeyHeaderHint: 'ID de tu cuenta Threatdown/Malwarebytes',
  },
  // Ejemplo para agregar otro conector en el futuro:
  // pfsense: {
  //   username: 'API Client ID',
  //   apiKey: 'API Secret',
  //   apiKeyHeader: 'Firewall Hostname',
  // },
}

function CredentialFields({ authType, connectorType, onChange }: {
  authType: AuthType
  ...
  // Obtener labels personalizados si existen para este tipo de conector
  const labels = connectorType ? (CREDENTIAL_LABELS[connectorType] ?? {}) : {}
  ...
  return (
    <div className="space-y-3">
      ...
      {authType === 'apikey' && (
        <>
          <div className="space-y-1.5">
            <Label>{labels.username ?? 'Username (optional)'}</Label>
            <Input
              placeholder={labels.usernamePlaceholder ?? 'apitoken'}
              onChange={(e) => onChange('username', e.target.value)}
            />
          </div>
          <div className="space-y-1.5">
            <Label>{labels.apiKey ?? 'API Key / Token'}</Label>
            <div className="relative">
              <Input
                type={showApiKey ? 'text' : 'password'}
                placeholder={labels.apiKeyPlaceholder ?? 'your-api-key'}
          ...
          <div className="space-y-1.5">
            <Label>{labels.apiKeyHeader ?? 'API Key Header (optional)'}</Label>
            <Input
              placeholder={labels.apiKeyHeaderPlaceholder ?? 'X-API-Key'}
              onChange={(e) => onChange('apiKeyHeader', e.target.value)}
            />
            <p className="text-xs text-muted-foreground">
              {labels.apiKeyHeaderHint ?? 'Default: X-API-Key'}
            </p>
          </div>
        </>
      )}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                      ...
                      {selectedType.authTypes.map(authType => (
                        <SelectItem key={authType} value={authType}>
                          {authType === 'basic' ? 'Basic Auth (username/password)' :
                           ...
                           authType === 'passhash' ? 'PRTG (username/passhash)' :
                           'None'}
                        </SelectItem>
                      ))}
                    </SelectContent>
              ...
              <div className="space-y-3 p-4 rounded-lg bg-secondary/30 border border-border/50">
                ...
                {/* ← connectorType pasado para labels personalizados */}
                <CredentialFields
